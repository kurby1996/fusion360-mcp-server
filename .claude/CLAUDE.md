...
1. **This repo** — Python MCP server (stdio transport, 103 tools). Claude talks to this.
...
- `src/fusion360_mcp/tools.py` — 103 tool definitions with annotations
...
- `tests/` — unit tests covering tools, mock handlers, server routing, connection, annotations
...
- All MCP length values are **millimetres**. The add-in converts to Fusion's internal centimetres. `execute_code` still uses centimetres.
